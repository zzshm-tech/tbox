                              ...
                              struct can_configure *cfg)
{
    ...
    filterConf = &drv_can->FilterConfig;
		if(drv_can->CanHandle.Instance == CAN1)
    ...
    filterConf->FilterFIFOAssignment = 0U;
...
//                HAL_NVIC_EnableIRQ(CAN1_RX1_IRQn);
        ...
        break;
			case RT_CAN_CMD_SET_FILTER:           					//硬件过滤
        /* TODO: filter*/
        break;
			case RT_CAN_CMD_GET_MSG_NUM:										//返回接收到CAN报文数量
				*(rt_uint32_t *)arg = can->status.rcvpkg;
				can->status.rcvpkg = 0;
				break;
...
static int drv_sendmsg(struct rt_can_device *can, const void *buf, rt_uint32_t boxno)
{
  ...
  hcan = &((struct stm32_drv_can *) can->parent.user_data)->CanHandle;
	if(pmsg->ide == RT_CAN_EXTID)//
	{
		...
		hcan->pTxMsg->RTR = pmsg->rtr << 1;
		hcan->pTxMsg->IDE = pmsg->ide << 2;
	}
	else
	{
    ...
    if(canHandle->Instance==CAN1)
    {
        ...
        */
        ...
        HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);
				//rt_kprintf("-- the config can gpio\r\n");
...
{
    struct stm32_drv_can *drv_can;
    struct can_configure config = CAN1DEFAULTCONFIG;

...
**	初始化系统时钟
*******************************************/
void rt_hw_sys_clock_init(void)
{
	//rt_device_t tmp_dev;

  SystemClock_Config();           //注意这个初始化系统运行始终
	//rt_rs232_power_on();
		
	//tmp_dev = rt_device_find("uart1");        //获取uart3句柄
  //rt_device_open(tmp_dev, RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_STREAM | RT_DEVICE_FLAG_INT_RX);  
}

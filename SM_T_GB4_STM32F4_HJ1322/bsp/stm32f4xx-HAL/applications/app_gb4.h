#ifndef __APP__ACL16__H__
#define __APP__ACL16__H__
...
//#define IPCODE_GETID	    0x11
#define IPCONDE_REBOOT    		0x11
/* CMD_CODE*/
#define CMDCODE_ID_GET		0x01	//获取chipid
...
#define TERIMNAL_TYPE ((uint8_t)1)   //不同数值代表不同型号，如0x01表示HOMER3X 0x02表示HOMER3E
/* END*/
/*********根据机型区分**************/
#define  DF_WC8W		0X01  //道法8W潍柴国四

#define WZ     			0XFE   // 
#define SLEEP_FRONT_WORK_TIME    30  //30s发送登出包
...
typedef struct signOrEncryData
{
	uint16_t dataLen;
	int8_t keyData[64];
	int8_t signOrEncryData[1024];
}SIGNOREN;
...
struct vehicle_msg_str
{
	...
	uint16_t 							engine_breakdown;					//发动机故障灯（新修改）
	uint16_t 							air_temp;									//大气温度
	uint16_t							oil_water_pilot;					//油中有水指示 （新修改）
	uint32_t							vehicle_work_time;					//发动机工作时间
	...
	uint16_t  						gearbox_out_rotate;   		//变速箱输出轴转速（新修改）
	...
	uint8_t 							drain_off_fault_class;	  //排放系统驾驶员严重程度指示灯（新修改）
	uint8_t 							dpf_build_light;							//DPF再生指示灯(新修改)
	uint8_t 							dpf_forbid_light;   		  		//DPF再生禁止灯（新修改）
	uint8_t 							clutch_status;								//离合开关状态
	uint8_t								fnr_active_light;							//FNR激活指示灯 （新修改）
	uint8_t 							auto_model_light;				  		//自动模式指示灯  （新修改）
	uint8_t 							power_dis_lilght;					    //动力切断指示灯  （新修改）
	uint8_t 							gearbox_filter_light;	  			//变速箱油滤堵塞指示灯  (新修改)
	uint8_t 							gearbox_switch;				  			//变速箱齿轮油压力 （变速箱档位）
	uint16_t 							gearbox_oil_temp;						  //传动系机油温度 （机械变速箱油温）
	uint16_t 							gearbox_oil_temp_e;		        //电控变速箱油温度	（新修改）				
	uint8_t 							engine_control_model;    			//发动机控制模式  (新修改)
	...
	uint16_t	   					daily_work_time;    					//日工作时间(TBOX计算)
	uint32_t 							tbox_vehicle_work_time;    		//整车工作时间（TBOX计算）
	int32_t  							vehicle_work_offset;    				//整车工作时间偏移量 也就是 发动机工作时间偏移量
	uint32_t 							engine_work_time;							//发动机工作时间原始值
	int32_t 							fuel_consume_offset;			//累计油耗偏移量
	uint32_t 							total_consume;       //累计油耗原始值
};
...
struct  obd1939_str
{	
	uint8_t					obd_protocol;				//OBD协议
	uint8_t					alarm_light_state;   //报警灯状态
};

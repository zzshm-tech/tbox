...
//extern int uart_cmd_handler(const char *data,rt_uint16_t len,rt_int32_t *arg);
...
#endif
#endif
//		if(uart_cmd_handler(cmd,length,&cmd_ret) == 0)
//		{
//			return cmd_ret;
//		}
		
		{
			struct rt_uart_cmd_event event;
			event.cmd = 0;
			event.arg_len = length;
			rt_memcpy(event.arg_buf,cmd,length);
			rt_mq_send(get_uart_cmd_mq(),&event,sizeof(event));
		}

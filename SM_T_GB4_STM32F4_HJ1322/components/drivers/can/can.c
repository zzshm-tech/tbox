...
static rt_err_t rt_can_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    {
				/**2020.5.21**/
				if(oflag & RT_DEVICE_FLAG_RADIO_TX)
				{
					int i = 0;
          struct rt_can_tx_fifo *tx_fifo;
            ...
            dev->open_flag |= RT_DEVICE_FLAG_RADIO_TX;
				}
    }
...
static rt_err_t rt_can_close(struct rt_device *dev)
{
    ...
    if (dev->open_flag & RT_DEVICE_FLAG_RADIO_TX)
    {
        struct rt_can_tx_fifo *tx_fifo;

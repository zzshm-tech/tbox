...
int list_for_str_b(uint8_t *buf,uint8_t *str,int datasize)
{
	unsigned char	 	ch;
	unsigned char		*str_cpy;
	int		 			index = 0;
...
uint32_t swap_uint32_tt(uint32_t n)
{
	uint32_t	 rv;
	
	*((uint8_t*)&rv + 0) = *((uint8_t*)&n + 2);
	*((uint8_t*)&rv + 1) = *((uint8_t*)&n + 3);
	*((uint8_t*)&rv + 2) = *((uint8_t*)&n + 0);
	*((uint8_t*)&rv + 3) = *((uint8_t*)&n + 1);
...
uint8_t  uds_crc8(uint8_t oldcrc8,uint8_t *pdata,uint32_t len)
{
	uint32_t        i;
	int             j;
	uint8_t         crc = oldcrc8;
...
void uds_security_key_cal(uint8_t *pseed,uint8_t *pkey)
{
	//seed len is 4byte
	//key len is 16byte
	//there level is const 4
	uint8_t i;
	//level %=5;
	uint8_t  level = 4;
    ...
    for (i = 0; i < 4; i++)
	{
		pkey[i * 2]=((pseed[i]) & (0x0F)) + ((i + level) << 4);
		pkey[i * 2 + 1]=((pseed[i]) & (0xF0)) + (i + level);
	}
	for (i = 0; i < 4; i++)
	{
		pkey[i * 2 + 8]=uds_crc8(pkey[i * 2],pseed,4);
		pkey[i * 2 + 9]=uds_crc8(pkey[i * 2 + 1],pseed,4);
	}
}

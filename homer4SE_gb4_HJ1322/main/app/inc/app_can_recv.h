...
struct back_data_t
{
	uint8_t 							manu_type;           						//ecu厂家  
	uint8_t 							mon_status;			//激活状态
	uint8_t 							lock_status;    //实际锁车状态
	uint8_t 							key_status;
	uint8_t 							id_status;            //GPSID匹配状态
	uint8_t 							lock_preparative_status;
	uint8_t								wc_lock_bank;   //实际锁车状态
...
struct ecu_data_str
{
	...
	uint8_t 							engine_torque_mode;							//发动机扭矩模式
	...
	uint32_t							engine_work_time;							//发动机工作时间.
	uint32_t 							engine_total_revolutions;     				//发动机总转速
	uint8_t 							accelerator_percent;						//加速踏板行程值
	uint8_t 							water_out_temp;								//
	uint16_t 							engine_fuel_instant;						//瞬时油耗-燃油经济性	 Km/L
	uint16_t							engine_fuel_average;						//平均油耗				Km/L
	uint32_t 	   					    total_fuel;  								//累计油耗
	...
	uint8_t 							dfp_inlet_pressure;  						//微粒捕集器入口压力  (颗粒捕捉器上游压力)
	uint16_t 							exhaust_gas_temp;							//排气温度
	uint8_t 							entered_air_intake_temp;					//发动机进气歧管温度
	uint8_t 							air_intake_prossure;						//
	uint8_t     						lock_preparative_status;					//ECU锁车状态
	uint8_t								mon_status;   								//锁车功能状态
	uint8_t 							key_status;     							//KEY码状态
	uint8_t 							id_status;									//TBOX ID状态
	uint8_t 							lock_status;								//实际锁车状态
	uint8_t 							urea_box_temp;								//尿素温度
	uint8_t 							urea_concentration;							//尿素浓度
	uint8_t								urea_percent;   							//尿素液位
	...
	uint8_t 							assist_stalling_sign;						//辅助熄火
	uint8_t 							intervene_engine_mode;  					//发动机控制
	uint16_t 							gearbox_oil_temp;							//变速箱油温
	uint16_t 							braking_air_pressure;						//制动气压
	uint16_t 							air_booster_out_temp;  						//发动机增压空气冷却器出口温度
	...
	uint8_t 							energy_saving_state;						//节能状态
	uint8_t 							clutch_status ;								//离合开关状态
	uint8_t 							brake_status;								//刹车开关状态
	uint8_t 							brake_status_light;							//
	...
	uint8_t 							dpf_surplus_light;							//DPF再生提醒灯
	uint8_t								dfp_build_light;							//DPF主动再生状态指示灯
	uint8_t								dpf_forbid_light;							//DPF的再生禁止指示灯
	uint8_t								dpf_build_fault;							//系统故障抑制DPF的活性再生判断灯
	uint8_t								dpf_temp_light;								//排气系统高温灯
	uint8_t 							dpf_dustiness;								//DPF积尘量
	...
	uint8_t								electronic_buff_torque;    					//电子缓冲器扭矩
	uint8_t 							mil_light_state;         					//排放报警灯状态;
	uint8_t 							manu_type;           						//ecu厂家  
	uint8_t 							res_state;                  				//ECU有关锁车的应答状态
	...
	uint8_t 							hmi_mon_state;						//仪表防拆激活状态
	uint8_t 							hmi_hbstate;						//仪表心跳校验状态

	uint16_t 							max_ref_torque;
};
...
struct md1                     
{
	...
	uint8_t             data[1][166];            //对应电控单元的总字节长度，直接单包CAN数据，为 报数N*8  ,最多5包数据，8个故障码
	uint8_t             led_state;              //
} ;
...
struct can_msg_str
{
    uint8_t 			can_status;
    uint32_t 			cnt;
    twai_message_t 		rx_msg[CAN_RX_MSG_NUM];/* 根据总线负载率设置最好 */
};
